...
;copy-ast :: ast -> ast
;return copy from ast
(define (copy-ast ast)
  ast)

(test (copy-ast (add (num 5) (num 6)))
      (add (num 5) (num 6)))

;print-ast :: ast -> void
;print-ast in the terminal
(define (print-ast ast)
  (print ast))

(test (print-ast (num 5))
      (void))


(define (interp-tree tree)
  (interp tree))
;grow :: int -> ast
...
(define (make-child mother father)
  (let ([new-child (crossover mother father)])
    (mutation new-child)
    ))
